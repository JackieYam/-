...
nolots = 0
# 先建立交易員帳號群組
for accEntry in cursor.execute("Select user_id, alias from trade_account \
    where exchange == 'TWSE'"):
    tuser = accEntry[0]
    tacc = accEntry[1]
    ...
    if not os.path.exists(inFile):
        ...
        if FileTime == strToday:
            for db in data.index:
                ...
                iLots = int(strLots)
                iOnwayLots = iLots - int(data.loc[db,'BI03260'])
                if iLots == 0:
                    continue
                ...
                checkPosition(strKey, strCode, strAcc, strAlias, iLots, iOnwayLots, iunrealize, irealize, iCost)
            pass
